...
class UpdateCurriculumRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'code' => ['required', 'string', 'max:50', Rule::unique('curriculums', 'code')->ignore($curriculumId)],
            'name' => ['required', 'string', 'max:255'],
            'start_year' => [
                'required',
                'integer',
                'between:2000,2100',
                Rule::unique('curriculums', 'start_year')
                    ->where(fn ($query) => $query->where('major_id', $this->input('major_id')))
                    ->ignore($curriculumId),
            ],
